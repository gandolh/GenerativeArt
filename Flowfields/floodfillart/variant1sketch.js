...
function setup() {
    ...
    background(51);
    initColoredArray();
    activeCells[0]=new ActiveCell([random(255),random(255),random(255)],createVector(int(random(rows)),int(random(cols))));
   for(let i=0;i<5;i++) activeCells.push(new ActiveCell([random(255),random(255),random(255)],createVector(int(random(rows)),int(random(cols)))));
}	
function draw(){
	for(let i=0;i<=300;i++){
	if(activeCells.length>0){
		...
		if(ok(xIndex,yIndex) && coloredCells[xIndex][yIndex]==0 ){
			let variation=3;
			let tempColor=[
				randomGaussian(ac.colr[0],variation)+120,
